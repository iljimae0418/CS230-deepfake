...
''' function to plot loaded numpy data ''' 
def plotImage(path):
    data = np.load(path)
    plt.imshow(data)
    plt.show()
...
''' preprocess data '''
frames_dir = './real_frames/'
real_frames = [frames_dir + x for x in os.listdir(frames_dir)]
real_frames.sort()
info = [] # array of tuples

saveDir = './frame_by_frame/'

fwa_frames = './fwa_frames/'

cnt = 0
none = 0

for path in real_frames:
    if (path == "./real_frames/datatable.csv"):
        continue
    print("Processing real frame {}".format(cnt))
    cnt = cnt+1
    query = re.search('./real_frames/(.*).npy',path)
    face_name = query.group(1)
    data = np.load(path)
    data = Image.fromarray(data.astype(np.uint8),'RGB')
    data.save(saveDir+face_name+'.jpg')
    face = cv2.imread(saveDir+face_name+'.jpg')
    face = cv2.cvtColor(face,cv2.COLOR_BGR2RGB)
    fwa_face = FWA(face,45,0)
    if fwa_face is None:
        print("FWA generation failed for frame {}".format(cnt))
        none = none+1
    np.save(fwa_frames+face_name+"_fwa"+".jpg",fwa_face)

print("Could not generate FWA for {} frames".format(none))
